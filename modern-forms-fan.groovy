...
int fanSpeedEnumToValue(fanSpeedEnum) {
    ...
    log.error("Unknown fan speed enum: ${fanSpeedEnum}, falling back to default (3)")
    return 3
}
...
void sendCommandToDevice(jsonBodyMap, callback) {
    try {
	httpPostJson(deviceUrl(), jsonBodyMap, callback)
    } catch (Exception e) {
	log.error("Could not fetch current fan state: ${e}")
    }
}
